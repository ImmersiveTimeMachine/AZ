...
5. **NEVER save / do editor-init-dependent work in the static init — it CRASHES editor STARTUP.** Confirmed 2026-06-22: an uncleaned generated `.cpp` whose ctor called `FEditorFileUtils::PromptForCheckoutAndSave` got compiled into the next full build; on editor launch the static init runs during `FEngineLoop::LoadStartupModules` (PreInit, before the editor can save) → hard crash, "editor can't start", every launch. Fix: `rm` the generated `.cpp` + full rebuild. The static init runs at MODULE LOAD (PreInit on full-build startup; patch-apply on Live Coding) — only do reflection/`LoadObject`/in-memory mutation there; mark dirty and SAVE FROM PYTHON afterwards (`EditorAssetLibrary.save_loaded_asset`). Also: Live Coding will NOT compile a brand-new generated file mid-session (it only patches files present in the last full build) — the snippet silently no-ops until a full build picks it up (then its static init runs at startup, per above).
