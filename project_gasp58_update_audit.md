...
## ★ Interaction DRIVER, read node-by-node 2026-08-31 (SandboxCharacter_Mover_Ragdoll)
- **Pawn-side, game thread, NOT the ABP availability path.** `IA_Takedown` → custom event `TryCharacterInteraction(InteractionType)`
  → gate `CanDoCharacterInteraction` = MovementMode==Walking(enum 4) && !IsSlotActive(DefaultSlot) && !WantsToAim →
  function `Try_MultiCharacterInteraction(InteractionType, OtherActors[])`:
  1. `EvaluateChooser CHT_CharacterInteractionPSDs({InteractionType, Speed})` → PSD array + `S_..._OUT{InitiatorRoles, TargetRoles}` per row.
  2. Per PSD: one `FPoseSearchMotionMatchMultiQuery{Database, AnimContextsRoles=[(self AnimInstance, InitiatorRole), (each other actor's AnimInstance, TargetRole)]}`.
  3. `UPoseSearchInteractionLibrary::MotionMatchMulti(Queries, PoseHistoryName="PoseHistory")` → `MMResults[]` (one FPoseSearchBlueprintResult PER ACTOR: SelectedAnim=PSIA, Role, RoleIndex, SelectedTime, WantedPlayRate, **ActorRootTransforms[]** (aligned transforms for every role), AnimContexts[]).
  4. For each result: `PoseSearchLibrary::GetActor(result)` → cast pawn → custom event `PlayCharacterInteractionAnimation(result)` on THAT pawn (sets `MMI_CurrentResult`, resolves the role montage from the PSIA, **aligns with `CustomTeleport`** (= Actor Teleport + QueueNextMovementMode Falling + OrientationIntent from the new rotation), plays the montage with StartingPosition=SelectedTime, PlayRate=WantedPlayRate via the PlayMontage proxy); if the actor is not self: `SkeletalMesh->AddTickPrerequisiteComponent(initiator mesh)` + `IgnoreComponentWhenMoving` both capsules.
- ABP side (`SandboxCharacter_Mover_ABP`): `Update_MMIConstraints`/`Update_InteractionConstraints` = `Get_MMIResult(owner)` → PSIA → montage active? → `UpdateConstraints(...)` + `GetConstraint(role, "hand_l"/"hand_r")` → IK alpha/targets (mesh-relative). `Get/Set_InteractionTransform` = a plain EssentialValues transform.
- Notifies: `BP_NotifyState_MMI_MotionWarping` = stock `AnimNotifyState_MotionWarping` with `RootMotionModifier_SkewWarp{WarpTargetName="WarpTarget", translation+rotation, IgnoreZ}` (who sets "WarpTarget" was not located in the pawn's FName table — verify at build time); `BP_NotifyState_MMI_IK` = window that enables the constraint IK.
- Source clips: `M_relaxed_takedown_*_A/V_` have bEnableRootMotion=true, bForceRootLock=true, RootLock=RefPose, montage RateScale 1.2; attacker 4.37 s, victim 1.90 s (victim is a "get grabbed" one-shot, attacker carries the whole move).
- Tools that read this: `AZ_BlueprintNodeUtils.list_function_nodes(bp, fn)` (functions only, NOT custom events), rider `ue_export_blueprint_nodes` (harness truncates >~8 KB; pass `nodeNames`), and `grep -a -o` on the .uasset for exact FNames (found `PlayCharacterInteractionAnimation`, `CustomTeleport`, `PlayMoverMontageCallbackProxy`).
