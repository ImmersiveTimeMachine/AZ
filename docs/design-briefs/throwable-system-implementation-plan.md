...
**Explicit September17 requirements:** selecting/equipping the grenade immediately enables its held idle before RMB; it belongs in the **Equippables** inventory category. Explore equipped carry and held Preparing/Aiming must reuse upper/lower-body layering with live lower-body idle/walk/run/crouch and stance transitions. These are mandatory, superseding any suggestion to root the player during long-lived carry/aim. Distinguish equipped carry from the throw action; keep count unchanged until physical release. The [current work order, section0](C:/UnrealEngine/Games/AZ/docs/design-briefs/claude-throwable-completion-work-order.md) covers canonical categorization, all selection routes and explicit acceptance cases.
...
- Permit normal grounded walking during preparation/hold. Sprint, jump, traversal or a weapon/item change cancels preparation cleanly. Release/recovery has authored interruption boundaries; death/grab/stagger retain priority.
...
On successful selection/equip, blend into the carried-item idle immediately without RMB, reservation or preview. Return to it after cancel/recovery while the selected stack remains; clear it on depletion/deselection. In Explore use existing upper-body layering over the live locomotion base for both carry and held preparation/aim. Verify walk, run, stops/turns, crouch idle, crouch movement and stance changes in the final rendered graph. A sequence assignment alone is not sufficient, and a downstream FullBody loop must not replace the legs. Handle any pelvis-dependent short release separately; do not impose its full-body behavior on long-lived carry/aim.

Keep grounded Mover locomotion running beneath the in-place upper-body action. The current graph's `RifleFire` slot in `WeaponFire` already serves weapon/equipment presentation; assess reuse with mutual exclusion. An indefinitely held `FullBody` montage would override legs and disable current procedural feet. RootMotionFromEverything is active, so accidentally choosing an RM variant is unsafe even without queuing a new movement task.
