# Claude — throwable completion requirements

**Primary entry point:** [September18 implementation handoff](C:/UnrealEngine/Games/AZ/docs/design-briefs/claude-throwable-implementation-handoff.md). Use its verified state and execution order, then this document's detailed requirements. The dedicated Throwable branch is now wired, compiled and saved, and production Quiet Sage art is assigned. Current Carry/Start/Loop montage tracks still use RifleFire; stance selection/lock, Mover action lock and Run cancellation remain unfinished. Earlier white-box visuals are historical findings, not a request to recreate art or the completed graph branch.
...
**Current stance/exclusivity policy from the newly submitted questions:** [numbered review response](C:/UnrealEngine/Games/AZ/docs/design-briefs/throwable-open-questions-review-response.md). Standing aim/release uses exclusive FullBody; crouch retains the masked upper-body route; Run cancels aim before Sprint dispatch; equipped carry remains movable. Capture/lock stance per action. This supersedes the older movable-standing-aim wording. Preserve the user-approved origin-lift tunable, while separating it from grip calibration and sweeping the real-grip→offset path.
...
- Equipping/selecting immediately activates held idle; grenade belongs in **Equippables**. Equipped carry preserves animated lower-body idle/walk/run/crouch. Under the newer reported policy, standing aiming is locked FullBody; crouched aiming keeps the upper-body mask over the crouched base. Do not confuse carry movement with exclusive aim movement.
...
These are required behavior, not optional polish. The original category/pose evidence is historical. Latest direct reads in `C:/UnrealEngine/Games/AZ/Saved/ThrowOpenQuestionsReview/placed-and-root-readback.json` confirm all seven loaded L_001 grenade pickups are already Equippable with the correct type tag. Do not migrate those again. Carry now uses CarryMontage; a legacy CarryPose assignment alone does not establish final-graph movement.
...
Apply this to equipped carry. For active aim, use the newer per-stance policy: standing FullBody with an explicit owner-scoped movement lock; crouched upper-body mix over the live crouched base. The mix does not itself grant permission to move while aim is exclusive. Keep the base state/feet coherent and avoid frozen carry legs, accidental FullBody carry or forced IdleLoop. Retain1× playback. Run exits aim before Sprint eligibility; it must not be blocked from requesting that cancellation.

The current unarmed source turns the pelvis substantially during release, so do not blindly mask the complete release clip above the waist. Standing FullBody and crouched masked variants need separate presentation routes and release anchors. Validate/adapt crouched content; do not silently substitute standing legs, force standing or add a universal pelvis counter-yaw. Authored footwork is not proof that capsule root motion is being driven. Keep carry independent from this exclusive action.
...
**Acceptance:** grenade appears under Equippables; inventory Equip and bound-slot/QuickSelect selection immediately show held idle without RMB or spending. Equipped carry supports idle/walk/run/stop/crouch and stance transitions. While holding RMB, standing uses locked FullBody and crouch retains its captured crouched/masked presentation; stance toggles do not switch route. Run cancels before Sprint dispatch. Cancel/recovery restores valid carry with no stale montage/count error. User performs gameplay checks after a successful build; no new automated tests.
...
The user explicitly requested the lift tunable; preserve it and its current prototype values (latest loaded Close50cm/Far−10cm). It is a declared world-space spawn offset, not arc curvature. Correct unlifted grip calibration separately, apply the same frozen offset to preview/release, and sweep from the real grip to the displaced origin as well as checking capsule/launch safety. Never use the offset to bypass a blocker. A missing live grip fails closed. Do not move the real projectile onto the preview path or silently delete/reinterpret the approved control.

Complete carry and the latest stance-specific action policy under section0 and the numbered review. **Movable carry, standing exclusive FullBody, crouched masked presentation** are distinct. Capture stance, lock new toggles, and release gameplay ownership promptly for Run cancellation. Do not treat the superseded movable-standing-aim recommendation as current.
