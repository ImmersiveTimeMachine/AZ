...
2. **`execute_script` exposed in tool list** — Node bridge `tool-router.js` reclassified from HIDDEN to SIMPLE. If `ToolSearch select:...execute_script` returns nothing, the bridge edit was reverted; restart Codex via `/mcp` after re-applying.
...
Full diffs and re-apply instructions: `C:\Users\Artur\.Codex\projects\C--UE57-Games-AZ\memory\project_local_plugin_patches.md` §1, §1b, §1c.
