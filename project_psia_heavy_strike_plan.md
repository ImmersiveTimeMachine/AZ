...
## 2026-09-07 — first-strike ENSURE fixed (PoseSearchInteractionAsset.cpp:54) + unreal-mcp fallback tooling
- Symptom: debugger break on the FIRST punch of a session, stack FindReferenceOrientation <- CalculateWarpTransforms <- CalculateFullAlignedTransform <- TryStrikeSearch. It is `ensure(CurrentWeightSum > 1e-8)` — once per session, NOT a crash; the strike still lands afterwards (engine falls back to Identity reference orientation).
- Cause: FindReferenceOrientation sums the ROTATION weights of every role except the heaviest; our PSIAs were attacker 1/1, victim 0/0 -> sum 0. Only a MIXED zero/non-zero rotation weighting trips it. All-zero (AZ_Catch_Fight: attacker 0/0, victim 0/1) takes the homogeneous branch (0.5 each) and is SAFE — no grab change needed.
- Fix: victim WarpingWeightRotation 0 -> 0.01 on AZ_Strike_Heavy_F5 / Kick_F5 / Jab_L_F5 / Jab_R_F5 (translation stays 0); Tools/strike_*_content_build.py updated with the reason. Per-role weights only shape the reference frame (final = AssetRoot x AssetRef^-1 x ActorsRef, no per-role weight), so yaw-only alignment is unchanged — VERIFIED 05:21: heavy pair LIVE, hero d=50 dyaw 0, root-root 126->110, fist->chest 39cm, no ensure.
- TRAP confirmed: editing a PSIA property PreCancels its database index ("PSD_AZ_Punch_R PreCancelled because of AZ_Strike_Jab_R_F5" seconds after the set). Reopen each DB editor -> "BuildIndex Succeeded / stored to DDC" (4/4); close the tabs before PIE. A dead index = every strike silently falls back to unpaired.
- Tooling: with unrealclaude disconnected, the OFFICIAL unreal-mcp server did the whole edit: ObjectTools.get_properties/set_properties (send the FULL Items array as JSON string; camelCase keys; object refs as {"refPath": ...}) + AssetTools.save_assets (verify by file mtime) + EditorAppToolset.OpenEditorForAsset (triggers the PoseSearch index build). It has no Chooser/PoseSearch-specific tools, but generic property edit + save + open-editor covers PSIA/DB maintenance.
