...
## 5. Engine/Mover — `bSkipInterpolation` teleport-snap flag (crouch proxy-pop fix)

**Why:** Mover interpolates the capsule on smoothed/proxy views, but `FStanceModifier` applies the crouch visual offset instantly. On crouch the capsule re-centers by the half-height delta (standing 90 − crouch 55 = **35cm**); the instant offset vs the lerped re-center desync → the character pops ±35 on **smoothed remote-pawn views** (the listen-server host's view of a remote client). CMC solves the identical case explicitly (`CharacterMovementComponent.cpp:3342-3351`, comment "Don't smooth this change in mesh position"); Mover lacked the guard. This patch implements Mover's equivalent (and Epic's own TODO in `FMoverDefaultSyncState::Interpolate`): a one-shot per-state flag that makes interpolation SNAP to a teleported state. Crouch needs ZERO project-side code — `FStanceModifier::AdjustCapsule` already queues an `FTeleportEffect`, which now carries the flag. Applied 2026-06-08. See [[project_crouch_system]].

**Files (NetworkPrediction/sync path only — if you switch to ChaosMover, patch `FTeleportEffect::ApplyMovementEffect_Async` too):**

| File | Change | Tag |
|---|---|---|
| `Engine/Plugins/Experimental/Mover/Source/Mover/Public/MoverDataModelTypes.h` (~line 172, in `FMoverDefaultSyncState`, after `MoveDirectionIntent`) | add `uint8 bSkipInterpolation = 0;` (non-UPROPERTY; copied by Clone's copy-ctor, manually net-serialized) | `// AZ PATCH (crouch proxy-pop fix)` |
| `Engine/.../Private/MoverDataModelTypes.cpp` `FMoverDefaultSyncState::NetSerialize` (after `Orientation.SerializeCompressedShort`) | `Ar.SerializeBits(&bSkipInterpolation, 1);` | same |
| `Engine/.../Private/MoverDataModelTypes.cpp` `FMoverDefaultSyncState::Interpolate` (the TeleportThreshold `if`) | add `ToState->bSkipInterpolation \|\|` to the snap condition | same |
| `Engine/.../Private/DefaultMovementSet/InstantMovementEffects/BasicInstantMovementEffects.cpp` `FTeleportEffect::ApplyMovementEffect` (after the `SetTransforms_WorldSpace` call) | `OutputSyncState.bSkipInterpolation = 1;` | same |

**4 sites — grep `AZ PATCH (crouch proxy-pop fix)`.** Naturally one-shot (the sim builds a fresh sync state each tick), so NO clear is needed.

**★★ CRITICAL GOTCHA (cost two full builds): do NOT clear `bSkipInterpolation` in `SetTransforms_WorldSpace`.** The first cut did, for "one-shot" — but the movement mode's **floor-snap calls `SetTransforms_WorldSpace` again right after the crouch teleport**, wiping the flag before it replicated → the patch was a total no-op even with Mover confirmed-rebuilt. The flag is already one-shot without any clear.
