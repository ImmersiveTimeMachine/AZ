...
- [feedback_verify_never_presume.md](feedback_verify_never_presume.md) — ★★★ MUST RULE: never presume, always check. No claim without a fresh read of the AUTHORITATIVE source, named in the answer; verify a change LANDED (not that the call succeeded); say "not verified" rather than fill a gap. Authoritative-source table inside (pin vs struct, LC log lines, CDO vs C++ default, notify reads).
- [feedback_stop_the_patch_loop.md](feedback_stop_the_patch_loop.md) — ★★★ USER RULE: no blind iteration. TWO-STRIKE rule (after 2 failed fixes STOP and re-model, don't patch); write the falsifiable prediction BEFORE the fix; ONE variable per test; ALWAYS diff against the last known-good log (a working land regressed unnoticed); never request a PIE without naming the pass/fail line; escalate the DESIGN not the patch count.
- [feedback_posesearch_mm_mechanism_rules.md](feedback_posesearch_mm_mechanism_rules.md) — ★★★ MUST RULES for MM selection, from 5 bugs in one feature: continuing-pose exemption keeps a clip SEARCHABLE not SELECTED (got wrong twice); anything that must play at a MOMENT must not be in a cost contest — narrow the pool; BranchIn = the INDEXED range; size BlockTransition to the clip's real role; KeepPlayingOneShotSearchable re-adds AFTER your strip; log the POOL first. Read before touching any MM pool, notify or gate.
- [feedback_model_routing_policy.md](feedback_model_routing_policy.md) — ★★ USER RULE: which model runs which task. Opus 5 = judgement (diagnosis, design, review); Sonnet 5 = mechanical/bulk/search subagents; Fable 5 only when Opus provably struggles; Haiku BANNED here (Feb 2025 cutoff predates UE 5.8). Tune `effort` before switching models; no subagent for a commit.
...
- [feedback_abp_internal_graph_blindspot.md](feedback_abp_internal_graph_blindspot.md) — ★ ListAnimGraphNodes misses the MM blend-stack bound graph (Steering/OW live there); OW alpha = clip curve Enable_Warping, missing = silently OFF; check curves exist before trusting curve-driven nodes.
...
- [project_cmc_mm_content_verdict.md](project_cmc_mm_content_verdict.md) — ★★★ 2026-08-27 AUDIT VERDICT: MM fidelity gap is CONTENT, not code — the -5 ContinuingPoseCostBias is INERT (clip notify hard-overrides it), 0 ExcludeFromDatabase vs GASP's 682, loops DB = ONE clip (69% of enterable poses are one-shots). Read BEFORE any further MM tuning.
- [project_cmc_jump_build_order.md](project_cmc_jump_build_order.md) — ★★ APPROVED 2026-08-27 CMC jump plan: GAS+capsule already done; Start clips CARRY the air (measured); per-gait Start/Land DBs = the stop pattern; all 24 AnimPro_Jump* clips mapped; 2 P0 defects to fix first (no ground guard on the stop contract, lying empty-gate fallback). Read FIRST for jump.
- [project_cmc_input_gap_doctrine.md](project_cmc_input_gap_doctrine.md) — ★★★ 2026-08-27 THE WORKING DESIGN (user: "ok works!!!"): "zero input" ≠ "player stopped" — ownership (`IsAnimDrivingMovement`, `IsPlayingRootMotion` is INERT for slot montages) + grace (`StopPoolGraceSeconds`, gate the POOL not the state) + no-gap handback. Acceptance metrics, the `[CmcSel]` frame-skew trap, PSD/normalization-set rules. Read FIRST for any CMC locomotion or turn work.
- [project_cmc_chooser_spine_landed.md](project_cmc_chooser_spine_landed.md) — ★★★ 2026-08-27: BlendStack+CHT spine WORKS Mover-clean; two-selector root cause + 4 fixes (MM-node hard-off, intent grace 10f, loop-only ContinuingProperties, sprint-row null repair); open: stop RED, sprint untested, leans off. Read FIRST on the spine.
- [project_cmc_velocity_master_verdict.md](project_cmc_velocity_master_verdict.md) — ★★★ START HERE for locomotion (thru 69c1a6e, 2026-08-26 night): VERDICT + Phase 1+2a+2b LANDED — SM-owned selection + per-sample dynamic play rate + pick-once via ContinuingPoseCostBias -5 (pool-narrowing and interrupt-mode are NOT pick-once — measured), eff ~1.00 GREEN on starts/stops/pivots; MM baseline FROZEN per user ("last attempt"). NEXT: build the BlendStack+CHT spine (Mover-identical) and A/B vs this baseline. Supersedes the RM-locomotion direction below.
- [project_cmc_rm_locomotion.md](project_cmc_rm_locomotion.md) — ★ SUPERSEDED for locomotion by the verdict above (mechanism keeps for committed actions; toggles: bRootMotionStarts/Stops). Was (commit 343d553): RM starts/stops via dynamic montages, 60% hand-back contract, stop entry-speed bands, the two foot bugs (FootSpeed_* curves don't exist; _LU means right-foot-planted), MM gating on a game-thread snapshot, CHT hooks additive.
- [project_cmc_turn_day_2026-08-24.md](project_cmc_turn_day_2026-08-24.md) — ★★ TURN DAY (commit 919ff9f): Enable_Warping was OFF on all clips; Get_TrajectoryTurnAngle mis-port (caps ~33°); TIP input-zero contract UNTESTED; pivot/starts gates, cascade bias, lean turn-budget, rotation 165 flat / friction 8 flat. Read FIRST for turn/TIP/selection work.
