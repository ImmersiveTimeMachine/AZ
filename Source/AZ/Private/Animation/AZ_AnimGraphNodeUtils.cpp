...
bool UAZ_AnimGraphNodeUtils::RebuildBlendSpace(UBlendSpace* BlendSpace)
{
#if WITH_EDITOR
...
FString UAZ_AnimGraphNodeUtils::AddAnimGraphNode(const FString& BlueprintPath, const FString& NodeClassPath,
	int32 PosX, int32 PosY)
{
#if WITH_EDITOR
	UAnimBlueprint* ABP = LoadAnimBP(BlueprintPath);
	UAnimationGraph* Graph = ABP ? FindAnimGraph(ABP) : nullptr;
	UClass* NodeClass = LoadObject<UClass>(nullptr, *NodeClassPath);
	if (!Graph || !NodeClass || !NodeClass->IsChildOf(UAnimGraphNode_Base::StaticClass())
	...
	int32 PosX, int32 PosY)
{
	...
	BSGraph->AddNode(Node, false, false);
	Node->PostPlacedNewNode();
	// Dispatch through the public base interface: the reference node's override is private.
	// Initial pin allocation needs no reconstruction (which is unsafe during Python authoring).
	static_cast<UEdGraphNode*>(Node)->AllocateDefaultPins();
	...
	const FString& PropertyName, const FString& BlendStackNodeGUID)
{
	...
	for (FOptionalPinFromProperty& OptPin : AnimNode->ShowPinForProperties)
	{
		...
		{
			// Compiling alone does not rebuild editor pins. Materialize this scalar property
			// directly so Python authoring never needs ReconstructNode (which can trigger GC).
			if (!AnimNode->FindPin(FName(*PropertyName), EGPD_Input))
			{
				FStructProperty* NodeProperty = FindFProperty<FStructProperty>(AnimNode->GetClass(), TEXT("Node"));
				FProperty* Property = NodeProperty ? NodeProperty->Struct->FindPropertyByName(FName(*PropertyName)) : nullptr;
				const UEdGraphSchema_K2* Schema = GetDefault<UEdGraphSchema_K2>();
				FEdGraphPinType PinType;
				if (!Property || Property->ArrayDim != 1 || CastField<FArrayProperty>(Property)
					|| !Schema->ConvertPropertyToPinType(Property, PinType)) return false;
				AnimNode->Modify();
				UEdGraphPin* Pin = AnimNode->CreatePin(EGPD_Input, PinType, FName(*PropertyName));
				void* NodeData = NodeProperty->ContainerPtrToValuePtr<void>(AnimNode);
				FString DefaultValue;
				Property->ExportTextItem_Direct(DefaultValue, Property->ContainerPtrToValuePtr<void>(NodeData), nullptr, AnimNode, PPF_None);
				Schema->SetPinAutogeneratedDefaultValue(Pin, DefaultValue);
			}
			AnimNode->Modify();
			OptPin.bShowPin = true;
			ABP->MarkPackageDirty();
			UE_LOG(LogTemp, Log, TEXT("AZ_AnimGraphUtils: Exposed pin %s.%s"),
