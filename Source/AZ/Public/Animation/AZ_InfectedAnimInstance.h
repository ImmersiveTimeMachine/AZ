...
/**
 * UAZ_InfectedAnimInstance — v2 AnimInstance for the Mover-driven Chalkie infected NPC pawn.
 *
 * INDEPENDENT COPY of UAZ_MoverAnimInstance (the player's AnimInstance): same v2 MM pipeline, but its own class
 * so the infected can diverge freely while the hero's AnimInstance stays untouched. Casts to
 * AAZ_PawnMoverInfectedCharacter. Once both are concrete, the shared parts can be promoted into a common base
 * ("see if/how it could be merged"); kept fully separate for now by design.
 *
